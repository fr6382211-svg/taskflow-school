...
## Perbaikan jadwal otomatis

Versi terbaru menambahkan `supabase/migrations/0002_default_schedule.sql`. Jalankan file ini sekali di **Supabase → SQL Editor**. Setelah itu, ketika user login dan membuka Jadwal/Kalender, database akan mengisi jadwal awal secara idempoten hanya jika tabel `schedule` masih kosong. Tidak ada hardcoded schedule di UI.

### Productivity migration
Setelah migration sebelumnya berhasil, jalankan sekali:
`supabase/migrations/0003_productivity_upgrade.sql`

Jangan menjalankan migration yang sama berulang kali pada project yang sudah menerapkan perubahan kolom/tabel, kecuali melalui migration runner/CLI.

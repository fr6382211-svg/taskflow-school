-- TASKFLOW v1.8 settings hardening.
...
alter table public.settings drop constraint if exists settings_theme_mode_check;
alter table public.settings
...
alter table public.settings drop constraint if exists settings_density_check;
alter table public.settings
...
set
  theme_mode = case when dark_mode then 'dark' else 'system' end,
  density = case when compact_mode then 'compact' else coalesce(density, 'comfortable') end
where theme_mode is null or density is null;

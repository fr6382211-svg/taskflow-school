...
Deno.serve(async (req) => {
  ...
  // SECURITY: fail-closed. Function ini verify_jwt=false (dipanggil oleh cron),
  // jadi CRON_SECRET WAJIB ada dan cocok. Tanpa secret yang terkonfigurasi,
  // tolak semua request daripada membiarkan endpoint terbuka untuk publik.
  if (!CRON_SECRET) {
    console.error('[daily-digest] CRON_SECRET belum diset di environment. Menolak semua request.');
  ...
  for (const target of targets ?? []) {
    ...
    if (existing) {
      results.push({ userId: target.user_id, skipped: true });
      continue;
    }
    ...
    try {
      ...
      results.push({ userId: target.user_id, sent: true, taskCount: tasks.length, lessonCount: lessons.length });
    } catch (sendError) {
      results.push({ userId: target.user_id, sent: false, error: sendError instanceof Error ? sendError.message : String(sendError) });
    }
  }

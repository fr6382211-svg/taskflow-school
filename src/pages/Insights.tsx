...
import { loadFocusStats, type FocusStats } from '../services/focusService';
...
export default function Insights({admin=false}:{admin?:boolean}) {
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      try {
        const result: FocusStats = await loadFocusStats(user.id);
        if (active) setFocusMinutes(result.totalMinutes);
      } catch (error) {
        console.error('Insights focus statistics load failed:', error);
        if (active) setFocusMinutes(0);
      }
    };

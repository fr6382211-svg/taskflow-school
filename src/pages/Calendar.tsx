...
export default function Calendar() {
  ...
  useEffect(()=>{if(!user?.id)return;const lastDay=new Date(year,month+1,1);const from=`${year}-${String(month+1).padStart(2,'0')}-01T00:00:00+07:00`;const to=`${lastDay.getFullYear()}-${String(lastDay.getMonth()+1).padStart(2,'0')}-01T00:00:00+07:00`;void supabase.from('focus_sessions').select('id,task_id,minutes,started_at,ended_at').eq('user_id',user.id).gte('started_at',from).lt('started_at',to).order('started_at').then(({data})=>setFocusRows((data??[]) as typeof focusRows));},[user?.id,year,month]);
  ...
  const selectedTutoring = tutoring.filter((item) => item.scheduleDate === selected);
  const selectedFocus = focusRows.filter((row) => new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Jakarta'}).format(new Date(row.started_at)) === selected);
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_380px]">
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 space-y-4">
          ...
          {selectedTutoring.length>0 && <section><div className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">Tutoring</div>{selectedTutoring.map((item)=><div key={item.id} className="mb-1 flex items-center justify-between rounded-lg bg-violet-50 px-3 py-2 dark:bg-violet-950/20"><div><div className="text-xs font-bold">{item.subjectName}</div><div className="text-[10px] text-slate-500">{item.activityType} • {item.className}</div></div><div className="text-[10px] font-bold text-violet-600">{item.startTimeLabel}</div></div>)}</section>}
          {selectedFocus.length>0 && <section><div className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">Focus sessions</div>{selectedFocus.map((row)=><div key={row.id} className="mb-1 flex items-center justify-between rounded-lg bg-emerald-50 px-3 py-2 dark:bg-emerald-950/20"><div className="text-xs font-bold">{row.minutes} menit fokus</div><div className="text-[10px] font-bold text-emerald-700">{new Intl.DateTimeFormat('id-ID',{timeStyle:'short'}).format(new Date(row.started_at))}</div></div>)}</section>}
          ...
          {selectedTasks.length===0 && selectedHoliday.length===0 && selectedTutoring.length===0 && selectedFocus.length===0 && !isWeekend(selected) && selectedSchedule.length===0 && <div className="rounded-xl bg-slate-50 p-3 text-sm text-slate-500"><CircleAlert size={15} className="mr-2 inline" />Tidak ada agenda lain.</div>}
        </div>
      </Card>
    </div>
  </div>;

...
import { BarChart3, CalendarDays, CheckSquare, Home, MoreHorizontal, Brain, Bell, MessageSquare, Search, Settings, UserRound, MapPin, Heart, MonitorPlay, Users, Activity, CheckCircle2 } from 'lucide-react';
...
const more = [
  ...
  { to: '/location-intelligence', label: 'Location Intelligence', icon: MapPin },
...
export default function MobileNav() {
  ...
  const extra = useMemo(() => isAdmin ? [...more, { to: '/admin', label: 'Admin Center', icon: Users }] : more, [isAdmin]);
  ...
  return <>
    <nav className="mobile-nav safe-bottom" aria-label="Navigasi utama seluler">
      {primary.map(({to,label,icon:Icon}) => <NavLink key={to} to={to} className={({isActive}) => `mobile-nav__item ${isActive ? 'is-active' : ''}`}>
        <Icon size={19}/><span>{label}</span>
      </NavLink>)}
      <button type="button" className={`mobile-nav__item ${isMoreActive || open ? 'is-active' : ''}`} onClick={() => setOpen((v) => !v)} aria-expanded={open} aria-haspopup="menu">

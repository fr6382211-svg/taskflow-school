// SECURITY: no hardcoded fallback key — must be provided via env var and
// restricted (API + HTTP referrer restrictions) in Google Cloud Console,
// since this key ships inside the public JS bundle.
const GOOGLE_CALENDAR_API_KEY = ((import.meta.env.VITE_GOOGLE_CALENDAR_API_KEY as string | undefined) ?? '').trim();

...
export async function listWorkspaceUsers(workspaceId:WorkspaceId, q=''):Promise<ChatMember[]> {
  let query=supabase.from('users').select('id,name,email,photo_url,workspace_id,status').eq('workspace_id',workspaceId).eq('status','active').order('name').limit(100);

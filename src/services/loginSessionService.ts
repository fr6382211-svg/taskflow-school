...
// Let a user sign a specific device of their own out remotely — the security-center
// action that was previously only possible for admins via /admin/sessions.
export async function revokeLoginSession(sessionId: string, userId: string): Promise<void> {
  const { error } = await supabase
    ...
    .update({
      ...
      revoked_reason: 'user_revoked_remote',
    })
    .eq('id', sessionId)
    .eq('user_id', userId);
  if (error) throw error;
...
// Bulk "sign out everywhere else" — keeps the current device's session untouched.
export async function revokeOtherLoginSessions(userId: string, keepSessionKey: string | null): Promise<number> {
  let query = supabase
    .from('login_sessions')
    .update({
      is_current: false,
      logout_at: new Date().toISOString(),
      revoked_at: new Date().toISOString(),
      revoked_reason: 'user_revoked_remote_bulk',
    })
    .eq('user_id', userId)
    .eq('is_current', true)
    .is('logout_at', null);
  if (keepSessionKey) query = query.neq('session_key', keepSessionKey);

...
interface AuthContextValue {
  ...
  isAdmin: boolean;
  login: (e: string, p: string, r: boolean, workspace?: WorkspaceId) => Promise<void>;
  register: (n: string, e: string, p: string, w: WorkspaceId) => Promise<void>;
  logout: () => Promise<void>;
...
function mapUser(r: Record<string, unknown>): UserProfile {
  return {
    ...
    workspaceId: r.workspace_id === 'mazet' ? 'mazet' : 'fathur',
  };
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const heartbeat = () => {
      void touchCurrentLoginSession(userId).catch((error) => {
        console.debug('TASKFLOW session heartbeat skipped', error);
      });

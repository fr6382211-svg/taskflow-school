...
export interface UserProfile { uid:string; name:string; email:string; photoURL?:string; role:Role; status:UserStatus; workspaceId?:'fathur'|'mazet'; createdAt?:string; updatedAt?:string; lastLoginAt?:string; }
export interface Task { id:string; title:string; description:string; subjectId:string; subjectName:string; teacherName:string; createdBy:string; createdByName:string; assignedTo:string; dueDate:string; dueTime:string; dueAt?:string; status:TaskStatus; priority:Priority; fileUrl?:string; fileName?:string; fileType?:string; fileSize?:number; storagePath?:string; createdAt?:string; updatedAt?:string; completedAt?:string; submissionStatus?:string; notes?:string; tags?:string[]; }
export interface ScheduleItem { id:string; day:'Senin'|'Selasa'|'Rabu'|'Kamis'|'Jumat'|'Sabtu'|'Minggu'; startTime:string; endTime:string; subject:string; teacher?:string; location?:string; type:ScheduleType; active:boolean; createdAt?:string; updatedAt?:string; }
...
export interface Notification { id:string; userId:string; title:string; message:string; type:'deadline'|'task'|'comment'|'mention'|'focus'|'announcement'|'security'|'system'|'calendar'|'achievement'; read:boolean; createdAt?:string; actionUrl?:string; }
